...
############################
# LC Based Autoscaling group
############################
resource "aws_autoscaling_group" "this" {
  count = var.create_asg && false == var.create_asg_with_initial_lifecycle_hook && false == var.create_lt ? 1 : 0
  ...
  name_prefix = "${join(
    ...
    compact(
      [
        ...
        var.recreate_asg_when_lc_changes ? element(concat(random_pet.lc_asg_name.*.id, [""]), 0) : "",
      ],
    ),
  )}-"
...
########################################################
# LC Based Autoscaling group with initial lifecycle hook
########################################################
resource "aws_autoscaling_group" "this_with_initial_lifecycle_hook" {
  count = var.create_asg && var.create_asg_with_initial_lifecycle_hook && false == var.create_lt ? 1 : 0
...
############################
# LT Based Autoscaling group
############################
resource "aws_autoscaling_group" "this_with_lt" {
  count = var.create_asg && var.create_lt && false == var.create_lc && false == var.create_asg_with_initial_lifecycle_hook ? 1 : 0
  ...
  name_prefix = "${join(
    ...
    compact(
      [
        coalesce(var.asg_name, var.name),
        var.recreate_asg_when_lc_changes ? element(concat(random_pet.lt_asg_name.*.id, [""]), 0) : "",
      ],
    ),
  )}-"
  vpc_zone_identifier  = var.vpc_zone_identifier
...
# LC Based Autoscaling group with initial lifecycle hook
########################################################
resource "aws_autoscaling_group" "this_with_initial_lifecycle_hook_and_lt" {
  count = var.create_asg && var.create_lt && var.create_asg_with_initial_lifecycle_hook && false == var.create_lc ? 1 : 0
...
resource "random_pet" "lc_asg_name" {
...
resource "random_pet" "lt_asg_name" {
